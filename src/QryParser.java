...
public class QryParser {
	...
	private static Qry createOperator(String operatorName) {
		...
		// #near/n and #window/n.
		if((operatorName.contains("#near")) || (operatorName.contains("#NEAR"))) {
			int numStartPosition = 6;
			String distance = operatorName.substring(numStartPosition);
			operatorDistance = Integer.parseInt(distance);
			operator = new QryIopNear(operatorDistance);
			operator.setDisplayName(operatorName);
			return operator;
		}
		
		if ((operatorName.contains("#window")) || (operatorName.contains("#WINDOW"))) {
			int numStartPosition = 8;
			String distance = operatorName.substring(numStartPosition);
			operatorDistance = Integer.parseInt(distance);
	 ...
	 *             Query syntax error.
	 */
	public static Qry getQuery(String queryString) throws IOException,
	 ...
	 *             Query syntax error.
	 */
	private static Qry parseString(String queryString) throws IOException,
			IllegalArgumentException {
		...
		// Added for weights
		String queryOperator = substrings[0].trim();
		...
		while (queryString.length() > 0) {
			...
			if(queryOperator.equals("#WSUM")) {
				p = popWeights(queryString);
				((QrySopWSum)queryTree).appendWeights(p.getPopped());
				queryString = p.getRemaining().trim();
			} else if ((queryOperator.equals("#WAND")) || (queryOperator.equals("#wand"))) {
				p = popWeights(queryString);
				((QrySopWAnd) queryTree).appendWeights(p.getPopped());
				queryString = p.getRemaining().trim();
			}
	...
	static private PopData<String, String> popWeights(String argString) {
		String[] subStrings = argString.split("[ \\t\\n\\r]+", 2);
		String weight = subStrings[0];
		
		argString = subStrings[1];
		
		return new PopData<String, String>(weight, argString);
	}
